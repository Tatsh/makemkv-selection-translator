...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `src/`, `tests/`,
  or dependency/version changes in `package.json`. **Skip for**: workflows, CI config, `.claude/`,
...
- Changes span unrelated components (e.g. `src/Share.elm` and `.claude/agents/release.md`).
...
- Only Wiswa-managed files changed (workflows, `package.json`,
...
- No files under `src/` or `tests/` changed.
...
- Elm source file `src/Share.elm` → `share:`.
...
- Test files `tests/ShareTests.elm` → `tests/ShareTests:` (or `tests:` for multiple).
- Dictionary `.vscode/dictionary.txt` → `dictionary:` (only when committed alone).
- Top-level config (`package.json`) → `project:`.
